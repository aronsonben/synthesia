...
export default function Palettes({ tracks, user }: PalettesProps) {
  ...
  return (
    <div className="h-1/2 flex flex-col items-center justify-center bg-[#D7D0DF]">
      ...
      <div className="flex flex-col p-4">
        {userTracks.map((track) => (
          <div key={track.id} className="flex flex-col gap-2 p-4 my-2 border rounded-lg">
          <h4 className="font-bold">{track.title}</h4>
          <div className="flex flex-wrap">
            {track.colors.map((color, index) => (
            <div key={index} className="w-8 h-8" style={{ backgroundColor: color }}></div>
            ))}
          </div>
          </div>
        ))}
